...
def start(bot, trigger):
    ...
    if target in bombs:
        if trigger.group(1) == 'bomb':
            ...
            return
    if trigger.group(1) == 'bomb':
        message = ('Hey, ' + target + u'! Somebody has given you a bomb! You have \x022 minutes\x02 and \x025 wires\x02: Red, Yellow, Blue, White and Black. What wire should I cut? Don\'t worry, I know what I\'m doing ! (answer with "%scutwire color")' % bot.config.prefix.replace("\\", ""))
        bot.say(message)
        ...
        if bot.config.lang == 'ca':
            message = ('Ei, ' + target + u'! Sembla que alg̼ t\'ha posat una bomba... Tens \x022 minuts\x02 i \x025 cables\x02: Vermell, Groc, Blau, Blanc i Negre. Quin cable he de tallar? No pateixis, jo s̩ el que em faig! (respon amb "%stalla color")' % bot.config.prefix.replace("\\", ""))
            bot.say(message)
